"""
Maker-Taker Sniper Backtest -- Tasks 2.19 & 2.20

Evolves from the Contrarian Taker to capture maker rebates on winning exits.
The Contrarian proved that CVD+Z signals have raw edge (34.6% win rate vs 33.3%
breakeven), but double-taker fees destroyed realized R/R.  The Sniper fixes
this by using a MAKER limit order for take-profits while keeping the TAKER
market order for stop-losses.

Core Logic:
    Trend Filter (5-minute rolling mean of USD price):
        Price > 5-min Mean  ->  macro trend UP, only LONG entries allowed.
        Price < 5-min Mean  ->  macro trend DOWN, only SHORT entries allowed.

    Entry Signal (same as Contrarian, but filtered by trend):
        CVD > +$5M AND Z > 2.0 AND trend=DOWN  ->  SHORT (buy NO)
        CVD < -$5M AND Z < -2.0 AND trend=UP    ->  LONG  (buy YES)

    Take Profit (Maker):
        POST_ONLY limit order +4.0c from entry. Earns maker rebate on fill.

    Stop Loss (Taker):
        Market order at -2.0c. Pays taker fee for immediate exit.

Risk Controls:
    $25 Daily Macro Breaker (resets at 00:00 UTC).
    5-minute post-trade cooldown.

Usage:
    python sniper_backtest.py
    python sniper_backtest.py --data data/btc_ticks_20260301_20260307.csv.gz
"""
...
logger = logging.getLogger("sniper_backtest")
...
# Trend filter: 5-minute (300-second) rolling mean of USD price.
TREND_WINDOW_MS = 300_000  # 300 seconds in milliseconds.
...
class SniperTradeResult:
    ...
    entry_fee: float        # Taker fee on entry (always positive cost)
    exit_fee: float         # Positive = taker fee paid; negative = maker rebate earned
    outcome: str            # "TP", "SL", "BREAKER", "EOD"
    entry_idx: int
    exit_idx: int
    maker_rebate: float     # Maker rebate earned on this trade (0 for SL/BREAKER/EOD)
...
class DailyMetric:
    ...
    maker_rebates: float    # Total maker rebates earned this day


@dataclass
class SniperResult:
    """Full backtest output."""
    trades: list[SniperTradeResult] = field(default_factory=list)
    ...
    num_ticks: int = 0
    trend_filtered_entries: int = 0   # Entries blocked by trend filter


# ---------------------------------------------------------------------------
# Sniper Backtest Engine
# ---------------------------------------------------------------------------
...
def run_sniper_backtest(
    ...
    """
    Run the Maker-Taker Sniper backtest.

    Entry: Taker market order (pays taker fee).
    Take Profit: Maker limit order (earns maker rebate).
    Stop Loss: Taker market order (pays taker fee).

    Trend filter: 5-min rolling mean gates entry direction.
    """
    oracle = OfflineOracle()
    balance = starting_capital
    position: SniperPosition | None = None
    trades: list[SniperTradeResult] = []
    ...
    # Trend filter: rolling window of (timestamp_ms, price) with running sum.
    trend_window: deque[tuple[int, float]] = deque()
    trend_sum: float = 0.0
    trend_filtered_entries = 0
    ...
    logger.info(
        "Sniper Backtest: %d ticks over %.1f hours, capital=$%.0f, "
        "trade_size=$%.0f, TP=+%.1fc (MAKER), SL=-%.1fc (TAKER), "
        "CVD>$%.0fM, Z>%.1f, trend=5min",
    ...
    for tick in ticks:
        ...
        # --- Trend filter: maintain 5-min rolling price window ---
        # Use running sum for O(1) mean instead of O(n) on every tick.
        trend_window.append((tick.timestamp_ms, tick.price))
        trend_sum += tick.price
        cutoff = tick.timestamp_ms - TREND_WINDOW_MS
        while trend_window and trend_window[0][0] < cutoff:
            _, old_price = trend_window.popleft()
            trend_sum -= old_price

        # Compute 5-minute rolling mean of USD price.
        trend_mean = trend_sum / len(trend_window)
        trend_up = tick.price > trend_mean
        trend_down = tick.price < trend_mean
        ...
        # Need at least 300 seconds of data for the trend filter.
        if len(trend_window) < 2:
            continue

        # LONG signal: crowd panic-selling the bottom + trend is UP.
        if cvd < -cvd_threshold and z < -zscore_threshold:
            if not trend_up:
                # Trend filter blocks: macro trend is not UP.
                trend_filtered_entries += 1
                continue
            position = _open_long(
            ...
            total_taker_fees += position.entry_fee
            logger.info(
                "LONG ENTRY @ tick %d: CVD=$%s, Z=%+.2f, prob=%.4f, "
                "trend_mean=%.2f, price=%.2f, TP=%.4f (MAKER), SL=%.4f",
                tick_idx, f"{cvd:+,.0f}", z, fair_prob,
                trend_mean, tick.price, position.tp_prob, position.sl_prob,
            )

        # SHORT signal: crowd panic-buying the top + trend is DOWN.
        elif cvd > cvd_threshold and z > zscore_threshold:
            if not trend_down:
                # Trend filter blocks: macro trend is not DOWN.
                trend_filtered_entries += 1
                continue
    ...
    # --- End of data: close any open position (taker exit) ---
    if position is not None:
        trade, pnl, exit_fee_amount, rebate = _close_position(
    ...
    return SniperResult(
        ...
        trend_filtered_entries=trend_filtered_entries,
    )
...
def _open_long(
    ...
    """Open a LONG (buy YES) position via taker market order."""
...
def _open_short(
    ...
    """Open a SHORT (buy NO) position via taker market order."""
    no_price = 1.0 - prob
...
def _check_exit(pos: SniperPosition, fair_prob: float) -> str | None:
...
def _close_position(
    ...
    idx: int,
    balance: float,
) -> tuple[SniperTradeResult, float, float, float]:
    """
    Close a position and compute PnL.

    TP exits use MAKER pricing (earn rebate instead of paying fee).
    SL/BREAKER/EOD exits use TAKER pricing (pay taker fee).

    Returns (SniperTradeResult, net_pnl, exit_fee_amount, maker_rebate).
    exit_fee_amount is the taker fee paid (0 for TP).
    maker_rebate is the rebate earned (0 for SL/BREAKER/EOD).
    """
    is_maker_exit = (outcome == "TP")
    exit_fee_amount = 0.0
    rebate_earned = 0.0
    ...
    if pos.direction == "LONG":
        ...
        if is_maker_exit:
            # Maker TP: earn rebate instead of paying fee.
            rebate_earned = gross_proceeds * maker_rebate_rate(exit_prob)
            net_proceeds = gross_proceeds + rebate_earned
        else:
            # Taker SL/BREAKER/EOD: pay taker fee.
            exit_fee_amount = gross_proceeds * taker_fee_rate(exit_prob)
            net_proceeds = gross_proceeds - exit_fee_amount
    ...
    # For the trade record, exit_fee is negative when it's a rebate.
    reported_exit_fee = -rebate_earned if is_maker_exit else exit_fee_amount

    trade = SniperTradeResult(
        direction=pos.direction,
        entry_prob=pos.entry_prob,
        exit_prob=exit_prob,
        pnl_usdc=pnl,
        entry_fee=pos.entry_fee,
        exit_fee=reported_exit_fee,
        outcome=outcome,
        entry_idx=pos.entry_idx,
        exit_idx=idx,
        maker_rebate=rebate_earned,
    )
    return trade, pnl, exit_fee_amount, rebate_earned
...
# ---------------------------------------------------------------------------
# Tear Sheet (Task 2.20)
# ---------------------------------------------------------------------------

def print_sniper_tear_sheet(result: SniperResult) -> None:
    """Print the Maker-Taker Sniper tear sheet."""
    ...
    print(f"\n{border}")
    print("    MAKER-TAKER SNIPER BACKTEST -- TASKS 2.19 & 2.20")
    print(border)
    ...
    if total_trades > 0:
        print(f"    Rebate per Winning Trade:      ${result.total_maker_rebates / max(wins, 1):>+11,.4f}")
    print(divider)

    print("  TRADE PERFORMANCE")
    print(f"    Total Trades Taken:            {total_trades:>12d}")
    print(f"    Wins (Take Profit - MAKER):    {wins:>12d}")
    print(f"    Losses (Stop Loss - TAKER):    {losses:>12d}")
    ...
    print("  TREND FILTER")
    print(f"    Entries Blocked by Trend:       {result.trend_filtered_entries:>12d}")
    total_signals = total_trades + result.trend_filtered_entries
    if total_signals > 0:
        filter_rate = result.trend_filtered_entries / total_signals * 100
        print(f"    Filter Rate:                   {filter_rate:>11.1f}%")
    print(divider)
    ...
    if result.daily_metrics:
        ...
        print(f"  {'Date':<12s}  {'PnL':>10s}  {'Trades':>6s}  "
              f"{'W':>3s}  {'L':>3s}  {'WR%':>5s}  {'Rebates':>8s}  {'Brk':>3s}")
        print(divider)
        for d in result.daily_metrics:
            ...
            print(
                ...
                f"{d.wins:>3d}  {d.losses:>3d}  {wr:>4.0f}%  "
                f"${d.maker_rebates:>+7,.4f}  [{brk}]"
            )
        print(divider)
        ...
        total_rebates = sum(d.maker_rebates for d in result.daily_metrics)
        print(
            f"  {'TOTAL':<12s}  ${total_pnl:>+9,.2f}  {total_t:>6d}  "
            f"{total_w:>3d}  {total_l:>3d}  {total_wr:>4.0f}%  "
            f"${total_rebates:>+7,.4f}"
        )
        print(border)
        print()
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Maker-Taker Sniper Backtest -- Trend-Filtered Fee Capture"
    )
    parser.add_argument(
    ...
    print_sniper_tear_sheet(result)
